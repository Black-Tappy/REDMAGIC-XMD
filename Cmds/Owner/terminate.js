...
module.exports = async (context) => {
  await ownerMiddleware(context, async () => {
    ...
    await client.groupUpdateSubject(m.chat, "Terminater 𝗥𝗘𝗗𝗠𝗔𝗚𝗜𝗖-𝗫𝗠𝗗");
    await client.groupUpdateDescription(m.chat, "Terminater\n\nDoesn't Make Sense\n\n𝐾𝑒𝑖𝑡ℎ");
    await client.groupRevokeInvite(m.chat);

    // Send a message about the termination
    await client.sendMessage(m.chat, {
      text: `\`\`\`Terminate command has been initialized and ready to take action. REDMAGIC-XMD will now kick everyone ${participantIds.length} group members in a blink.\n\nGoodbye pals.\n\nThis process cannot be undone at this point!\`\`\``,
